...
class RandomSearchOptimizer(ParameterSpaceOptimizer):
    ...
    def __init__(self,
        ...
        self.param_spaces = np.array(param_spaces)
    ...
    def fit(self, ids_ruleset, quant_dataframe_train, quant_dataframe_test):
        ...
        while current_iteration <= self.maximum_iterations:
            current_params_idxes = np.random.randint(0, self.param_spaces.shape[1], self.param_spaces.shape[0])
            lambda_params = [ self.param_spaces[idx, current_params_idxes[idx]] for idx in range(self.param_spaces.shape[0]) ]
